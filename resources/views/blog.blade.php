                ...
                <div class="flex flex-col items-center sm:px-5 md:flex-row">
                    <div class="w-full md:w-1/2">
                        <a href="{{ route('blog.show', ['post' => $post->slug]) }}" class="block relative">
                            <img
                                class="absolute inset-0 -z-10 object-cover w-full h-full rounded-lg max-h-64 sm:max-h-96 blur-md"
                                src="{{ asset('storage/'.$post->image) }}" loading="lazy"
                                alt="Photo by {{ $post->author->name }}">
                            <img class="object-contain w-full h-full rounded-lg max-h-64 sm:max-h-96"
                                 src="{{ asset('storage/'.$post->image) }}" loading="lazy"
                                 alt="Photo by {{ $post->author->name }}">
                        </a>
                    </div>
                    <div class="flex flex-col items-start justify-center w-full h-full py-6 mb-6 md:mb-0 md:w-1/2">
                        <div
                            ...
                            <h1 class="text-4xl font-bold leading-none lg:text-5xl xl:text-6xl"><a
                                    href="{{ route('blog.show', ['post' => $post->slug]) }}">{{ $post->title }}</a></h1>
                            <p class="pt-2 text-sm font-medium">by <a
                                    href="{{ route('blog') }}?author={{ $post->author->id }}"
                                    class="mr-1 underline">{{ $post->author->name }}</a> · <span
                                    class="mx-1">{{ \App\Services\DateService::formatPublishedAtDate($post->published_at) }}</span>
                ...
                <div class="w-full sm:container mx-auto">
                    <div class="max-w-4xl mx-auto px-10 bg-white rounded-lg shadow-lg">
                        <div class="flex flex-col items-center justify-center py-6">
                            <img src="https://cdn-icons-png.flaticon.com/128/907/907717.png" alt="Welcome Icon"
                ...
                <div class="flex grid grid-cols-12 pb-10 sm:px-5 gap-x-8 gap-y-16">
                    ...
                    @foreach($posts->skip(1) as $post)
                        ...
                        <div class="flex flex-col items-start col-span-12 space-y-3 sm:col-span-6 xl:col-span-6">
                            <a href="{{ route('blog.show', ['post' => $post->slug]) }}" class="block relative">
                                <img class="absolute inset-0 -z-10 object-cover w-full mb-2 overflow-hidden rounded-lg shadow-sm max-h-96 blur-md"
                                     src="{{ asset('storage/'.$post->image) }}" loading="lazy"
                                     alt="Photo by {{ $post->author->name }}">
                                <img class="object-cover w-full mb-2 overflow-hidden rounded-lg shadow-sm max-h-96"
                                     src="{{ asset('storage/'.$post->image) }}" loading="lazy"
                                     alt="Photo by {{ $post->author->name }}">
                            </a>
                            ...
                            <h2 class="text-lg font-bold sm:text-xl md:text-2xl"><a
                                    href="{{ route('blog.show', ['post' => $post->slug]) }}">{{ $post->title }}</a></h2>
                            <p class="text-sm text-gray-500">{{ $post->excerpt }}</p>
                            <p class="pt-2 text-xs font-medium"><a href="{{ route('blog') }}?author={{ $post->author->id }}"
                                                                   class="mr-1 underline">{{ $post->author->name }}</a>
                                · <span
                                    class="mx-1">{{ \App\Services\DateService::formatPublishedAtDate($post->published_at) }}</span>
